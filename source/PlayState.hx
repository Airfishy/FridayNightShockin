...
class PlayState extends MusicBeatState
{
	...
	override public function update(elapsed:Float)
	{
		...
		if(ratingString == '?') {
			scoreTxt.text = 'Wacky Number: ' + songScore + ' | Fuck Ups: ' + songMisses + ' | U good: ' + ratingString;
		} else {
			scoreTxt.text = 'Wacky Number: ' + songScore + ' | Fuck Ups: ' + songMisses + ' | U good: ' + ratingString + ' (' + Math.floor(ratingPercent * 100) + '%)';
		}
